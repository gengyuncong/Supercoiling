...
def write_elongation_stall_2(f, index, k, name_dict, lk0, Inf, PCoil_threshold, NCoil_threshold):
	k = k-1; #python uses 0-index system
	Reactants=[name_dict['RNAP'][k], name_dict['Twist'][k+1], name_dict['Twist'][k-1]];
	Products=[name_dict['RNAP_stall'][k], name_dict['Twist'][k+1], name_dict['Twist'][k-1]];
	write_reaction_stall(f, index, Reactants, Products, lk0, Inf, PCoil_threshold, NCoil_threshold);

def write_elongation_resumption_2(f, index, k, name_dict, lk0, resumeRate, PCoil_threshold, NCoil_threshold):
	k = k-1; #python uses 0-index system
	Reactants=[name_dict['RNAP_stall'][k], name_dict['Twist'][k+1], name_dict['Twist'][k-1]];
	Products=[name_dict['RNAP'][k], name_dict['Twist'][k+1], name_dict['Twist'][k-1]];
	write_reaction_resume(f, index, Reactants, Products, lk0, resumeRate, PCoil_threshold, NCoil_threshold);

def write_premature_termination(f, index, k, name_dict, premature_rate, mRNA_index):
	k = k-1; #python uses 0-index system
	Reactants=[name_dict['RNAP_stall'][k]];
	Products=[name_dict['DNA'][k], name_dict['mRNA_pre'][k], name_dict['S_pre'][mRNA_index]];
	Propensity_sp=Reactants;
...
def write_initiation_2(f, index, name_dict, promoter_index, lk0, Boundary0, Boundary1, initiation_rate, mRNA_index):
...
def write_termination(f, index, name_dict, terminator_index, mRNA_index, Inf):
	...
	Products=[name_dict['DNA'][terminator_index-1], name_dict['S2'][mRNA_index]];
	Propensity_sp=Reactants;
	Propensity_para=Inf;
	write_reaction(f, index, Reactants, Products, Propensity_sp, Propensity_para);
...
def write_mRNA_degradation_initiation_2(f, index, name_dict, promoter_index, mRNA_degradation_rate):
	Reactants=[name_dict['mRNA'][promoter_index-1]];
	Products=[name_dict['mRNA_degr'][promoter_index-2]];
	Propensity_sp=Reactants;
	Propensity_para=mRNA_degradation_rate;
	write_reaction(f, index, Reactants, Products, Propensity_sp, Propensity_para);
...
def write_mRNA_degradation_elongation_2(f, index, name_dict, k, RNase_elongation_rate):
	k = k-1;
	Reactants=[name_dict['mRNA'][k], name_dict['mRNA_degr'][k], name_dict['mRNA_pre'][k]];
	Products=[name_dict['mRNA_degr'][k-1], name_dict['mRNA_pre'][k]];
	write_reaction_mRNA_degradation_elongation(f, index, Reactants, Products, RNase_elongation_rate);
...
loop_rate=0;
unloop_rate=0;
#loop_unloop=float(sys.argv[2]);
#loop_rate=float(sys.argv[1]);  # every 2 min
#unloop_rate=loop_rate/(1.0*loop_unloop);# every 10 min

# Gyrase
gyrase_binding_rate=0.0018*50;    # every 1s
#gyrase_binding_rate=0;
gyrase_unbinding_rate=0.5;
NCoil_producing_rate=1000;

# topoI
topoI_binding_rate=0.0018*50;
#topoI_binding_rate=0.0018;
#topoI_binding_rate=0;
topoI_unbinding_rate=0.5;
PCoil_producing_rate=1000;

# NCoil
relaxation_rate=50; #1,5,10,50
NCoil_initial=56;
rate1=56*relaxation_rate;
rate2=relaxation_rate;

#stall
#initiation_rate=0.1;
initiation_rate=float(sys.argv[1]);
initiation_list = [0.05];
initiation_list2 = [initiation_rate];
#initiation_list = [initiation_rate];
#initiation_list2 = initiation_list;
...
# global config:
#'''
inter_len = int(sys.argv[2]) #2,8,20,40,80,100

num=180; #240
barrier=[];
'''
promoter_list=[100-inter_len/2]; #61-inter_len/2, 60
terminator_list=[120-inter_len/2]; #80-inter_len/2, 79

promoter_list2=[140+inter_len/2]; #100+inter_len/2, 
terminator_list2=[120+inter_len/2]; #81+inter_len/2, 
'''
promoter_list=[70-inter_len/2]; #61-inter_len/2, 60
terminator_list=[90-inter_len/2]; #80-inter_len/2, 79

promoter_list2=[110+inter_len/2]; #100+inter_len/2, 
terminator_list2=[90+inter_len/2]; #81+inter_len/2,

Boundary0_list = [0];
Boundary1_list = [-0.06];
Boundary0_list2 = [0];
Boundary1_list2 = [-0.06];

#print(promoter_list)
#print(promoter_list2)
...
mRNA_index=[0];
mRNA_index2=[1];
name_dict['protein'] = ['protein1', 'protein2'];
name_dict['S1'] = ['S1_1', 'S1_2'];
name_dict['S2'] = ['S2_1', 'S2_2'];
name_dict['S_pre'] = ['S_pre_1', 'S_pre_2'];

###########################################
# write files
filename='conv.'+str(sys.argv[1])+'.'+str(sys.argv[2])+'.sbml'
f = open(filename, 'w')
wirte_head(f);
...
for i in range(0,len(name_dict['protein'])):
	name=name_dict['S_pre'][i];
	write_sp(f, name, 0);
...
for i in range(0,len(promoter_list)):
	...
	for k in range(promoter_i, terminator_i): #P, P+1, ..., T-1
		...
		write_premature_termination(f, index, k, name_dict, premature_rate, mRNA_i); index = index+1;
	...
	for k in range(promoter_i, terminator_i+1): #P, P+1, ..., T
	...
	for k in range(promoter_i+1, terminator_i-1): #P+1, P+2, ..., T-2
...
#'''
for i in range(0,len(promoter_list2)):
	promoter_i = promoter_list2[i];
	terminator_i = terminator_list2[i];
	initiation_rate_i = initiation_list2[i];
	Boundary0_i = Boundary0_list2[i];
	Boundary1_i = Boundary1_list2[i];
	mRNA_i = mRNA_index2[i];
	write_initiation_2(f, index, name_dict, promoter_i, lk0, Boundary0_i, Boundary1_i, initiation_rate_i, mRNA_i); index = index+1;
	for k in reversed(range(terminator_i+1, promoter_i+1)): #T-(-1), T-(-2), ..., P
		write_update_step(f, index, k, name_dict, elongation_rate); index = index+1;
		write_pre_elongation(f, index, k, name_dict, Inf); index = index+1;
		write_elongation1_2(f, index, k, name_dict, Inf); index = index+1;
		write_elongation_stall_2(f, index, k, name_dict, lk0, Inf, PCoil_threshold, NCoil_threshold); index = index+1;
		write_elongation_resumption_2(f, index, k, name_dict, lk0, resumeRate, PCoil_threshold, NCoil_threshold); index = index+1;
		write_premature_termination(f, index, k, name_dict, premature_rate, mRNA_i); index = index+1;
		write_elongation_stall_diffusion_D2U(f, index, k, name_dict, stall_diffusion_rate); index = index+1;
		write_elongation_stall_diffusion_U2D(f, index, k, name_dict, stall_diffusion_rate); index = index+1;
		write_elongation_translocation_diffusion_D2U(f, index, k, name_dict, stall_diffusion_rate); index = index+1;
		...
		write_elongation_translocation_diffusion_D2U_tmp(f, index, k, name_dict, stall_diffusion_rate); index = index+1;
		write_elongation_translocation_diffusion_U2D_tmp(f, index, k, name_dict, stall_diffusion_rate); index = index+1;
	for k in reversed(range(terminator_i, promoter_i+1)): #T, T-(-1), ..., P
		write_elongation3_2(f, index, k, name_dict, Inf); index = index+1;
		write_elongation4_2(f, index, k, name_dict, Inf); index = index+1;
	write_termination(f, index, name_dict, terminator_i, mRNA_i, Inf); index = index+1;
	write_mRNA_degradation_initiation_2(f, index, name_dict, promoter_i, mRNA_degradation_rate); index = index+1;
	write_mRNA_degradation_initiation_pre(f, index, name_dict, promoter_i, mRNA_degradation_rate); index = index+1;
	for k in reversed(range(terminator_i+2, promoter_i)): #T-(-2), T-(-3), ..., P+(-1)
		write_mRNA_degradation_elongation_2(f, index, name_dict, k, RNase_elongation_rate); index = index+1;
		write_mRNA_degradation_elongation_pre(f, index, name_dict, k, RNase_elongation_pre_rate); index = index+1;
	write_mRNA_degradation_termination(f, index, name_dict, terminator_i+1, RNase_elongation_rate); index = index+1;
	write_mRNA_degradation_elongation_pre(f, index, name_dict, terminator_i+1, RNase_elongation_pre_rate); index = index+1;
	write_translation(f, index, name_dict, terminator_i+1, mRNA_i, translation_rate); index = index+1;
...
for k in range(1,num):
	write_diffusion1(f, index, k, name_dict, diffusion_rate); index = index+1;
for k in range(2,num+1):
	write_diffusion2(f, index, k, name_dict, diffusion_rate); index = index+1;
...
for k in [1, num]:
	write_diffusion7(f, index, k, name_dict, rate1); index = index+1;
	write_diffusion8(f, index, k, name_dict, rate2); index = index+1;
