...
def write_termination(f, index, name_dict, terminator_index, mRNA_index, Inf):
	...
	Products=[name_dict['DNA'][terminator_index-1], 'S2'];
	Propensity_sp=Reactants;
	Propensity_para=Inf;
	write_reaction(f, index, Reactants, Products, Propensity_sp, Propensity_para);
...
# loop
loop_rate=0.0083*2; #every 1 min
unloop_rate=0.0083*2;
#unloop_rate=0.00166*2; #every 5 min
...
###########################################
# write files
filename='domain_v2.'+str(sys.argv[1])+'.sbml';
f = open(filename, 'w')
wirte_head(f);
