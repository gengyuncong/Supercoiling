...
# topoI
topoI_binding_rate=0.0018*float(sys.argv[2]);
...
num=149;
barrier=[];
promoter_list=[30]; #Z5 marker
terminator_list=[115];
signal_index=75;
Z3_index=80; #Z3 maker  
...
###########################################
# write files
filename='anta.'+str(sys.argv[1])+'_'+str(sys.argv[2])+'.sbml';
f = open(filename, 'w')
wirte_head(f);
...
#fp = h5py.File('ini'+str(sys.argv[2])+'.lm', "r");
#replicate=1;
#counts=fp["/Simulations/%07d/SpeciesCounts"%replicate];
#NCoil_amount=counts[-1,range(3*num,4*num)]

NCoil_amount=initialize_NCoil_amount(num, free_NCoil);

for i in range(0,num):					#3-4
	name=name_dict['Twist'][i];
	write_sp(f, name, NCoil_amount[i]);
